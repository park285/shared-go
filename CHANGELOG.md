...
- gRPC를 보안 수정 안정판 `v1.83.2`로 고정하여 `v1.84.0`에 적용되는
  `CVE-2026-84445` / `GO-2026-6443` 경고를 해소합니다. 개발 버전이나 `replace`는 사용하지 않습니다.
- 공개 API와 OpenTelemetry 버전은 유지하며, OTLP span 전송과 shutdown flush를 검증했습니다.
...
- Go 1.26 workspace가 선택한 OpenAI SDK, OpenTelemetry, gRPC와 `x/*` 의존성 버전을
  스택 소비자와 정합화했습니다.
...
- `workercontract` metric label 복제를 Go 1.26 `maps.Copy`로 정합화했습니다.
