...
PERF_GATE_COLLECT_ARGS := --policy scripts/perf/perf-budget.yaml --candidate $(PERF_GATE_CANDIDATE) --gate-id $(PERF_GATE_ID) --gate pr
...
perf-gate: perf-gate-test
	...
	./scripts/perf/check-bench-regression.sh --policy scripts/perf/perf-budget.yaml --baseline $(PERF_GATE_BASELINE) --candidate $(PERF_GATE_CANDIDATE) --gate-id $(PERF_GATE_ID) --gate pr

.PHONY: guard-perf-gate
guard-perf-gate:
	./scripts/perf/check-bench-regression.sh collect --policy scripts/perf/perf-budget-guards.yaml --candidate $(GUARD_PERF_CANDIDATE) --gate-id $(GUARD_PERF_GATE_ID) --gate pr --count 6 --benchtime 100ms
	./scripts/perf/check-bench-regression.sh --policy scripts/perf/perf-budget-guards.yaml --baseline $(GUARD_PERF_BASELINE) --candidate $(GUARD_PERF_CANDIDATE) --gate-id $(GUARD_PERF_GATE_ID) --gate pr
