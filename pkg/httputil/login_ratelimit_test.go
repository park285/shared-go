...
type loginFailureRateLimiterCase struct {
	name       string
...
func (tc loginFailureRateLimiterCase) run(t *testing.T) {
	t.Helper()

	allowed, retry, count := tc.action()
	if tc.checkCount {
	...
	if allowed != tc.wantAllow {
		t.Fatalf("allowed = %t, want %t", allowed, tc.wantAllow)
	}

	if (retry > 0) != tc.wantRetry {
		t.Fatalf("retry = %s, want positive=%t", retry, tc.wantRetry)
	}
}

func loginFailureRateLimiterCases(limiter *LoginFailureRateLimiter, identity string) []loginFailureRateLimiterCase {
	allow := func() (bool, time.Duration, int) {
		allowed, retry := limiter.IsAllowed(identity)

		return allowed, retry, 0
	}

	fail := func() (bool, time.Duration, int) {
		return false, 0, limiter.RecordFailure(identity)
	}

	return []loginFailureRateLimiterCase{
		{name: "initial allowed", action: allow, wantAllow: true},
		{name: "failure one", action: fail, wantCount: 1, checkCount: true},
		{name: "failure two", action: fail, wantCount: 2, checkCount: true},
		{name: "failure three locks", action: fail, wantCount: 3, checkCount: true},
		{name: "locked", action: allow, wantAllow: false, wantRetry: true},
	}
}

// 서브테스트가 하나의 limiter 상태를 순차로 진행시키므로 병렬화하지 않는다.
func TestLoginFailureRateLimiter(t *testing.T) {
	...
	for _, tt := range loginFailureRateLimiterCases(limiter, identity) {
...
func TestLoginFailureRateLimiterConcurrentReservationsHonorCapacity(t *testing.T) {
	...
	for i := range callers {
		identity := fmt.Sprintf("identity-%02d", i)
