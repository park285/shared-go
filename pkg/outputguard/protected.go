...
const (
	...
	rollingHashRuneMask    = rune(1<<21 - 1)
)
...
const (
	...
	exactIncomingBits   = 8
	exactIncomingMask   = uint64(1<<exactIncomingBits - 1)
	exactFailureReady   = uint64(1 << 56)
	exactTransitionMask = uint64(1<<exactFailureShift - 1)
	maxExactNodes       = 1 << exactFirstBits
)

// trie 노드는 패턴 바이트당 최대 1개이므로 노드 수는 maxProtectedTotalBytes+1을 넘지 않는다.
// 이 상수 선언은 그 상한이 노드 인덱스 필드 폭(maxExactNodes)을 넘으면 음수가 되어 컴파일을 막는다.
const _ = uint(maxExactNodes - maxProtectedTotalBytes - 1)
...
func buildExactTrieNode(nodes *[]exactNode, node uint32, patterns []string, depth int) {
	if len(patterns) == 1 {
		...
		for depth < len(pattern) {
			child := exactNodeIndex(len(*nodes))
			setExactTransitions(&(*nodes)[node], child, 1, false)
	...
	first := exactNodeIndex(len(*nodes))
...
func buildExactFailures(nodes []exactNode) {
	...
	for node := uint32(1); node < exactNodeIndex(len(nodes)); node++ {
...
func setExactTransitions(node *exactNode, first, count uint32, terminal bool) {
	if count == 0 {
		first = 0
	}
...
func exactNodeIndex(index int) uint32 {
	return uint32(index & (maxExactNodes - 1))
}
...
func exactCount(node exactNode) uint32 {
	return uint32(uint64(node) >> exactCountShift & exactCountMask)
}
...
func exactIncoming(node exactNode) byte {
	return byte(uint64(node) >> exactIncomingShift & exactIncomingMask)
}
...
func exactNext(nodes []exactNode, state uint32, value byte) (uint32, bool) {
	...
	for low < high {
		middle := (low + high) >> 1
		if exactIncoming(nodes[first+middle]) < value {
			low = middle + 1
		} else {
			high = middle
		}
	...
	if low < count && exactIncoming(nodes[first+low]) == value {
		return first + low, true
	}
...
func (index *protectedIndex) hasTokenOverlap(output []rune, outputTokens []tokenSpan) bool {
	for tokenStart := 0; tokenStart+tokenAnchorWindow <= len(outputTokens); tokenStart++ {
		...
		for _, ref := range refs {
			...
			outputStart, outputEnd := expandExactRunes(output, start, end, entry.runes, ref.start, ref.end)
...
func (index *protectedIndex) hasRuneOverlap(output []rune, _ []tokenSpan) bool {
...
func expandExactRunes(left []rune, leftStart, leftEnd int, right []rune, rightStart, rightEnd int) (int, int) {
...
func hashRunes(text []rune) uint64 {
	...
	for _, r := range text {
		hash = hash*rollingHashBase + uint64(r&rollingHashRuneMask) + 1
	}
...
func rollRuneHash(hash uint64, oldRune, newRune rune, power uint64) uint64 {
	return (hash-(uint64(oldRune&rollingHashRuneMask)+1)*power)*rollingHashBase + uint64(newRune&rollingHashRuneMask) + 1
}
