-- 이 문은 lock_reply_sequence.sql로 순번열을 잠근 뒤에만 정확하다. 후속 ordinal 가드가
-- 스냅샷 읽기이고 READ COMMITTED의 문 스냅샷은 잠금 획득 전에 잡히므로, 잠금과 삽입이 한 문에
-- 있으면 대기가 풀려도 방금 commit된 후속 행을 보지 못한다. 두 문으로 나눠야 두 번째 문이
-- 새 스냅샷을 얻는다.
--
