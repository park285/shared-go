...
type codedRetryableErr struct {
	msg       string
	code      string
	retryable bool
}

func (e *codedRetryableErr) Error() string   { return e.msg }
func (e *codedRetryableErr) Code() string    { return e.code }
func (e *codedRetryableErr) Retryable() bool { return e.retryable }
...
func TestErrorAttrs_CodedRetryable(t *testing.T) {
	cause := &codedRetryableErr{msg: "temporary", code: "TEMP", retryable: true}
...
func TestErrorAttrs_EmptyCodeOmitted(t *testing.T) {
	cause := &codedRetryableErr{msg: "x", code: "", retryable: false}
	m := attrsToMap(ErrorAttrs(cause))
	if _, ok := m["error_code"]; ok {
...
func TestErrorType_NamedTypeUsesName(t *testing.T) {
	if got := errorType(&codedRetryableErr{}); got != "codedRetryableErr" {
		t.Fatalf("errorType = %q, want codedRetryableErr", got)
	}
}
